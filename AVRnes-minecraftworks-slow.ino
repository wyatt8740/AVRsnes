...
void loop() {
  ...
  if (state & SNES_DOWN) {
    ...
    if(currentMode==0)
    {
      ...
      Keyboard.release('s'); //for minecraft
    }
  }
  if (state & SNES_LEFT) {
    if(currentMode==0)
    {
      ...
      Keyboard.press('a'); //for minecraft
    }
  }
  else {
    if(currentMode==0)
    {
      ...
      Keyboard.release('a'); //for minecraft
    }
  }
  if (state & SNES_RIGHT) {
    if(currentMode==0)
    {
      ...
      Keyboard.press('d'); //for minecraft
    }
  }
  else {
    if(currentMode==0)
    {
      ...
      Keyboard.release('d'); //for minecraft
    }
  }
  if (state & SNES_A) {
    if(currentMode==0)
  ...
  if ((state & SNES_R) && (state & SNES_SELECT) && currentMode==1) { //open inventory in minecraft (send 'e')
      Keyboard.press('e');
  }
  else {
      Keyboard.release('e');
  }
  if ((state & SNES_L) && (state & SNES_SELECT && currentMode==1)) { //pause minecraft (send 'escape' key)
